...
<script type="text/javascript">
  $(document).ready(function(){
    $("#subprjct_nm").change(function(){
      ...
      if (spid != "") {
        $.ajax({
          url:"<?php echo SITE_URL; ?>/basic/finance/bms_for_payasgn.php",
          data:{subprjct_nm:spid, joucd:<?php echo $_POST['jo_unqcd']; ?>},
          type:'POST',
...
<script type="text/javascript">
  $(document).ready(function(){
    $("#bmsnm").change(function(){
      ...
      if (subprj != "" && blngmsid != "") {
        $.ajax({
          url:"<?php echo SITE_URL; ?>/basic/finance/bms_val_for_payasgn.php",
          data:{subprjct_nm:subprj, bmsnm:blngmsid, joucd:<?php echo $_POST['jo_unqcd']; ?>},
          type:'POST',
          success:function(response) {
            var resp = $.trim(response);
	  ...
	  <table class="table table-bordered">
	    ...
	    <tbody>
	    	<tr>
	    		...
	    		<td>
	    			<input type="text" class="form-control" name="wrk_dscrptn" id="wrk_dscrptn" placeholder="Work Description" value="<?php if (isset($_POST['prqid'])) { echo $vsprow->wrk_dscrptn; } ?>" readonly>
	    		</td>
	    		<td>
	    			<div class="input-group">
		          <span class="input-group-addon"><i class="fa fa-rupee"></i></span>
		          <input type="text" class="form-control" name="subprjct_val" id="subprjct_val" value="<?php if (isset($_POST['prqid'])) { echo $vsprow->subprjct_val; } ?>" placeholder="Sub-Project Value" readonly>
		        </div>
	    		</td>
	    	</tr>
	    </tbody>
